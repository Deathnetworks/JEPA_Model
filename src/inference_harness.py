...
class InferencePipeline:
    def __init__(self, engine_path="jepa_engine.pth", decoder_path="latent_decoder.pth", tokenizer_name="Qwen/Qwen2.5-7B-Instruct"):
        ...
        try:
            self.engine.load_state_dict(torch.load(engine_path, map_location=self.device, weights_only=True), strict=True)
        ...
        try:
            self.decoder.load_state_dict(torch.load(decoder_path, map_location=self.device, weights_only=True), strict=True)
        ...
        for attempt in range(max_retries):
            ...
            with torch.no_grad():
                for t in range(0, seq_len, chunk_size):
                    ...
                    from src.runtime import get_autocast_kwargs
                    ...
                    from src.runtime import empty_cache
                    empty_cache()
